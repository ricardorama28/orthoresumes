# BIBLIOGRAFÍA — Ediciones cerradas

Estas son las ediciones de referencia por subespecialidad. **No se usan ediciones anteriores.** `revisor-clinico` verifica que las citas de cada tema coincidan con la edición que le corresponde a su módulo.

| SUBESPECIALIDAD | REFERENCIA (edición fija) |
|---|---|
| Trauma MS/MI agudo | *Rockwood & Green's Fractures in Adults*, **10.ª ed.** (2024/25) + **AO Surgery Reference** |
| Mano, muñeca, codo | *Green's Operative Hand Surgery*, **8.ª ed.** (2022) |
| Pie y tobillo | *Coughlin & Mann's Surgery of the Foot and Ankle*, **10.ª ed.** (2023) |
| Hombro | *Rockwood & Matsen's The Shoulder*, **6.ª ed.** (2021) |
| Columna | *Rothman-Simeone & Herkowitz's The Spine*, **8.ª ed.** (2024) + **AOSpine** |
| Ortopedia general | *Campbell's Operative Orthopaedics*, **15.ª ed.** (2026) — **nunca la 14.ª** |
| Rodilla y deporte | *Insall & Scott Surgery of the Knee*, **7.ª ed.** (2024) |
| Tumores | *WHO Classification of Tumours*, **5.ª ed.** (2020) + *OKU Musculoskeletal Tumors*, **5.ª ed.** (2024) |
| Currículo base | Temas COT/UDELAR **7.ª edición** (en `fuentes/cot/`) |

## Mapa módulo → referencia principal

| MÓDULO | REFERENCIA PRINCIPAL |
|---|---|
| GEN | Campbell's 15.ª (2026) · AO Surgery Reference · Rockwood Adults 10.ª |
| TUM | WHO 5.ª (2020) · OKU Musculoskeletal Tumors 5.ª (2024) |
| MSA | Rockwood Adults 10.ª · Green's Hand 8.ª · AO Surgery Reference |
| MSC | Rockwood & Matsen Shoulder 6.ª · Green's Hand 8.ª |
| MIA | Rockwood Adults 10.ª · Coughlin & Mann 10.ª · Insall & Scott 7.ª · AO Surgery Reference |
| MIC | Insall & Scott 7.ª · Coughlin & Mann 10.ª · Campbell's 15.ª |
| COL | Rothman-Simeone Spine 8.ª · AOSpine |

## Artículos

- Preferentemente de los **últimos 10 años**, con **PMID o DOI**.
- Los **clásicos fundacionales** se citan por nombre y año sin límite temporal. Lista no exhaustiva:
  - Gustilo-Anderson 1976 (clasificación de fracturas expuestas)
  - Mirels 1989 (score de fractura patológica inminente)
  - Patchell 2005 (cirugía + RT en compresión medular metastásica)
  - Neer 1970 (clasificación de húmero proximal)
  - Garden 1961 (cuello femoral)
  - Schatzker (platillos tibiales)
  - Lauge-Hansen / Weber (tobillo)
  - Frykman / Fernández (radio distal)

## Regla operativa para el redactor

Cada afirmación cuantitativa citable (umbral, ángulo, plazo, tasa) debería poder rastrearse a una de estas fuentes. Si una cifra no tiene respaldo en la edición cerrada del módulo, se marca como pendiente en `revisor-clinico`, no se inventa.
